            ...
            <div class="row">
                            ...
                            <table>
                                <tbody>
                                    ...
                                    @if (count($cartItems) == 0)
                                        <tr class="d-flex">
                                            <td style="width: 100%" class="wsus__pro_icon">
                                                Cart is empty!
                                            </td>
                                        </tr>
                                    @endif
                ...
                <div class="col-xl-3">
                    <div class="wsus__cart_list_footer_button" id="sticky_sidebar">
                        <h6>total cart</h6>
                        <p>subtotal: <span id="sub_total">{{ getSidebarCartTotal() }}{{ $settings->currency_icon }}</span></p>
                        <p>discount: <span id="discount">{{getCartDiscount()}}{{ $settings->currency_icon }}</span></p>
                        <p class="total"><span>total:</span> <span id="cart_total">{{getMainCartTotal()}}{{$settings->currency_icon}}</span></p>
                ...
                <div class="col-xl-6 col-lg-6">
                    <div class="wsus__single_banner_content">
                        ...
                        <a href="{{ $cartpage_banner_section->banner_one->banner_url }}">
                            <img class="img-fluid"
                                src="{{ asset($cartpage_banner_section->banner_one->banner_image) }}"
                                alt="">
                        </a>
                        @endif
                    </div>
                ...
                <div class="col-xl-6 col-lg-6">
                    <div class="wsus__single_banner_content single_banner_2">
                        ...
                        <a href="{{ $cartpage_banner_section->banner_two->banner_url }}">
                            <img class="img-fluid"
                                src="{{ asset($cartpage_banner_section->banner_two->banner_image) }}"
                                alt="">
                        </a>
                        @endif
                    </div>
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.status == 'success') {
                            let productId = '#' + rowId;
                            let totalAmount = data.price_product_total +
                            ...
                            $('.product-increment').addClass('disabled')
                    ...
                    success: function(data) {
                        if (data.status == 'success') {
                            ...
                            let totalAmount = data.price_product_total +
                                "{{ $settings->currency_icon }}";
                            $(productId).text(totalAmount);
                            $('.product-increment').removeClass('disabled')
                            renderCartSubTotal();
                            calculateCouponDiscount();
                            // toastr.success(data.message)
                        } else if (data.status == 'error') {
